...
class TestMindMapApp:
    ...
    def app(self, mock_config):
        with (
            patch("job_genie.core.orchestrator.get_llm_client"),
            patch("job_genie.core.orchestrator.ResumeService"),
            patch("job_genie.core.orchestrator.AnalysisService"),
            patch("job_genie.core.orchestrator.ReferralService"),
        ):
            return MindMapApp(str(mock_config))

    def test_map_all_networks(self, app):
        with patch("job_genie.core.orchestrator.JobDetailsExtractor") as mock_extractor_cls:
    ...
    def test_refresh_existing_jobs(self, app):
        with (
            patch("job_genie.core.orchestrator.JobDetailsExtractor") as mock_extractor_cls,
            patch("job_genie.core.orchestrator.BrowserManager"),
        ):
            mock_extractor = mock_extractor_cls.return_value
            mock_extractor.db.get_all_jobs.return_value = [{"id": "123", "title": "AI"}]
    ...
    def test_scrape_no_jobs(self, app):
        with (
            patch("job_genie.core.orchestrator.JobSearcher") as mock_searcher_cls,
            patch("job_genie.core.orchestrator.JobDetailsExtractor") as mock_extractor_cls,
            patch("job_genie.core.orchestrator.BrowserManager"),
        ):
    ...
    def test_search(self, app):
        with (
            patch("job_genie.core.orchestrator.JobSearcher") as mock_searcher_cls,
            patch("job_genie.core.orchestrator.JobDetailsExtractor"),
            patch("job_genie.core.orchestrator.BrowserManager"),
        ):
            mock_searcher = mock_searcher_cls.return_value
    ...
    def test_sync(self, app):
        with patch("job_genie.core.orchestrator.SyncService") as mock_sync_cls:
    ...
    def test_tailor_resume(self, app):
        with patch("job_genie.core.orchestrator.JobDetailsExtractor") as mock_extractor_cls:
            ...
            with patch("job_genie.core.orchestrator.ResumeTailorer") as mock_tailorer_cls:
