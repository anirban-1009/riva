...
4. **Draft the description in Markdown**:
   - **Title**: under 70 characters, describes the change not the mechanism ("Add X" not "Update files for X").
   - **Summary**: bullet points grouped by theme/subsystem, not one bullet per commit. Group related commits together even if they landed as separate commits. Focus on *what* changed and *why* it matters.
...
- Don't pad the summary with restatements of the diff stat ("added 5 files, changed 3 files").
