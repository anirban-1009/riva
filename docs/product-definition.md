...
[[Architecture]] now leads with memory and profile as the load-bearing layer, ahead of the plugin protocol. A Genie is a thing that reads and writes memory in a domain-shaped way.
...
Today [`gateway.py`](https://github.com/anirban-1009/riva/blob/main/src/riva_agent/api/gateway.py) is a **stateless proxy**: `chat_completions` reads `request.messages` and forwards it verbatim. Riva owns no conversation state and assembles no prompt.
...
- **Doc drift** — fixed: [[Architecture]] now marks unimplemented modules (`common/memory/`, `common/profile/`, `common/interfaces/plugin.py`, `common/llm/router.py`) as planned and leads with memory/profile ahead of the plugin protocol.
